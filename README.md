非常非常非常好的node入门，很清晰，很详细。。。。
本文转载Manuel Kiessling的Node入门    原文链接 http://www.360doc.com/content/16/1128/09/2340477_610091091.shtml
...
你也许会想花点时间读一下Felix Geisend?rfer的大作Understanding node.js，它介绍了一些背景知识。
...
我们需要的所有数据都会包含在request对象中，该对象作为onRequest()回调函数的第一个参数传递。但是为了解析这些数据，我们需要额外的Node.JS模块，它们分别是url和querystring模块。
           ...
           url.parse(string).pathname      |
                       ...
                       |                   |
                     ------ -------------------
http://localhost:8888/start.jpg"foo"]    |
                                            |
...
这里我们要用到的外部模块是Felix Geisend?rfer开发的node-formidable模块。它对解析上传的文件数据做了很好的抽象。 其实说白了，处理文件上传“就是”处理POST数据 —— 但是，麻烦的是在具体的处理细节，所以，这里采用现成的方案更合适点。
